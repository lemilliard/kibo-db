...
@app_admin.route("/database/<_db_system_name>", methods=["GET", "PUT", "DELETE"])
def db_endpoint(_db_system_name=None):
    return jsonify(DatabaseEndpoint.process_request(_db_system_name))


@app_admin.route("/database/<_db_system_name>/table", methods=["GET", "POST"])
@app_admin.route("/database/<_db_system_name>/table/<_tb_system_name>", methods=["GET", "PUT", "DELETE"])
